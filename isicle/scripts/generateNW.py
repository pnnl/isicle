...
class NWChemHelper:
    ...
    def dft(self, template, charge):
        ...
        d = {
            "filename": splitext(basename(self.file))[0],
            "dir": self.dir,
            "charge": int(charge),
        }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Perform the proper preparation steps for an NWChem job"
    )
    parser.add_argument("infile", help="path to input .xyz file")
    parser.add_argument(
        "--template", help="path to template .nw file", default="default"
    )
    ...
    if args.dft is True:
        ...
        if args.template == "default":
            args.template = resource_filename(
                "isicle", "resources/nwchem/shielding.template"
            )
