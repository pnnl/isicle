...
if __name__ == "__main__":
    ...
    if splitext(args.infile)[-1].lower() == ".inchi":
        mol = inchi2geom(s, forcefield=args.forcefield, steps=args.steps)
    elif splitext(args.infile)[-1].lower() == ".smi":
        mol = smiles2geom(s, forcefield=args.forcefield, steps=args.steps)
    else:
        raise IOError(
            'File type "%s" not supported.' % splitext(args.infile)[-1].lower()
        )
